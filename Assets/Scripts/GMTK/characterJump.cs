...
//You can transform the number on screen to the stat here, using the function at the bottom of this script
    ...
    [SerializeField, Range(0.2f, 1.25f)][Tooltip("How long it takes to reach that height before coming back down")] public float timeToJumpApex = 1f;
    ...
    private void setPhysics()
    {
        ...
        float gravityScale = (newGravity.y / Physics2D.gravity.y) * gravMultiplier;
        if (gravityScale == Mathf.Infinity || gravityScale == Mathf.NegativeInfinity)
        {
            Debug.LogWarning("Invalid gravityScale");
            return;
        }
